...
class VSJunior:
    def __init__(self, root):
        ...
        self.menu_formatar.add_command(
            ...
            command=lambda: Janela_procuraMuda(self.root, self.get_current_text_widget())
        )
    ...
    def abaNova(self, title):
        ...
        text_area.bind("<KeyRelease>", lambda event: self.highlight_syntax(text_area))
        text_area.bind("<KeyRelease>", lambda event: self.update_status(text_area), add=True)
        text_area.bind("<ButtonRelease>", lambda event: self.update_status(text_area))
